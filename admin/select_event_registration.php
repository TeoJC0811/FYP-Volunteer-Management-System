...
if ($role === 'organizer') {
    $sql .= " AND e.organizerID = " . intval($userID);
}

if (!empty($search)) {
    $searchTerm = $conn->real_escape_string($search);
    $sql .= "
        AND (e.eventName LIKE '%$searchTerm%' 
        OR e.eventLocation LIKE '%$searchTerm%'
        OR e.eventCountry LIKE '%$searchTerm%')
    ";
}
...
<head>
<meta charset="UTF-8">
<title>Select Event to Manage Registrations</title>
...
/* 🔍 FORUM STYLE SEARCH BAR (Center Aligned, Light Blue) */
.search-filter-bar {
    display: flex;
...
.search-filter-bar a.btn-clear:hover { background-color: #dee2e6; }

/* 📋 Event List & Cards - RETAINED */
.event-list { 
    max-width: 900px; 
    margin: 30px auto; 
    display: grid;
...
<div class="main-content">
    <h2><i class="fas fa-calendar-check"></i> Manage Registrations</h2>
    <p style="text-align: center; color: #555;">Dashboard for <?php echo strtoupper($role); ?>: Manage participant details for your hosted events.</p>

    <div class="search-filter-bar">
        <form method="get" action="" style="display: flex; width: 100%; gap: 10px; align-items: center; justify-content: center;">
            <input type="text" name="search" placeholder="Search event name, location..." value="<?= htmlspecialchars($search) ?>">
            
            <select name="status">
                <option value="upcoming" <?= $statusFilter === 'upcoming' ? 'selected' : '' ?>>Upcoming & Current</option>
            ...
            <a href="select_event_registration.php" class="btn-clear"><i class="fas fa-undo"></i> Clear</a>
        </form>
    </div>
        ...
        <?php if ($result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
                ...
                <div class="event-card">
                    ...
                    <h3><?= htmlspecialchars($row['eventTitle']); ?></h3>
                    ...
                    <div class="event-details">
                        ...
                        <div class="detail-row">
                            ...
                            <span><?= htmlspecialchars($row['eventLocation']); ?>, <?= htmlspecialchars($row['eventCountry']); ?></span>
                        </div>
                    </div>
                    ...
                    <a href="manage_event_registration.php?eventID=<?= $row['eventID']; ?>" class="btn-manage">
                        <i class="fas fa-user-edit"></i> Manage Registrations
                    </a>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <p class="no-results">No events found matching your filters.</p>
        <?php endif; ?>
    </div>
</div>
</body>
