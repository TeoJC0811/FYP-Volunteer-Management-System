...
if (!$courseID) {
    header("Location: manage_training.php?error=NoTrainingSelected");
    exit();
}
...
if (!$course) {
    header("Location: manage_training.php?error=TrainingNotFound");
    exit();
}

if ($userRole === 'organizer' && $course['organizerID'] != $userID) {
    header("Location: manage_training.php?error=UnauthorizedAccess");
    exit();
}
...
/* ==========================
    FETCH SELECTED PAST COURSES
========================== */
$selectedPastCourses = [];
$sp = $conn->prepare("SELECT pastCourseID FROM coursepast WHERE courseID = ?");
$sp->bind_param("i", $courseID);
$sp->execute();
$resSp = $sp->get_result();
while ($r = $resSp->fetch_assoc()) {
    $selectedPastCourses[] = $r['pastCourseID'];
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $location = trim($_POST['courseLocation']);
    $country = trim($_POST['courseCountry']);
    ...
    if ($startDateTime < $today) {
        ...
        $error = "⚠️ Deadline cannot be later than the start date.";
        $isError = true;
    }

    if (!$isError) {
        $pastCourseIDs = [];
        if (!empty($_POST['pastCourses'][0])) {
            $pastCourseIDs = array_map('intval', explode(",", $_POST['pastCourses'][0]));
        }
        ...
        $certName = $course['certificate_template'];
        ...
        if (!$isError) {
            ...
            $stmt->bind_param(
                ...
                $startDate, $endDate, $start, $end, $deadline,
                $maxParticipant, $fee, $coverImageName, $categoryID, $certName,
                $courseID
            );

            if ($stmt->execute()) {
                /* UPDATE PAST TRAINING LINKING */
                $conn->query("DELETE FROM coursepast WHERE courseID = $courseID");
                if (!empty($pastCourseIDs)) {
                    $ins = $conn->prepare("INSERT INTO coursepast (courseID, pastCourseID) VALUES (?, ?)");
                    foreach ($pastCourseIDs as $pid) {
                        $ins->bind_param("ii", $courseID, $pid);
                        $ins->execute();
                    }
                }
                ...
                header("Location: edit_training.php?id=$courseID&msg=updated");
                exit();
            } else {
...
if(isset($_GET['msg'])) {
    if($_GET['msg'] == 'updated') $message = "✅ Training course updated successfully!";
...
<div class="main-content">
    <div class="back-link-wrapper">
        <a href="manage_training.php" class="back-link">← Back to Manage Training</a>
    </div>
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <div class="form-row">
                <div>
                    ...
                    <input type="text" name="courseLocation" value="<?= htmlspecialchars($course['courseLocation']) ?>" required>
                </div>
                <div>
                    <label>Country <span class="req">*</span></label>
                    <select name="courseCountry" required>
                        <?php foreach ($countries as $c): ?>
                            <option value="<?= $c ?>" <?= $course['courseCountry']==$c?'selected':'' ?>><?= $c ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            ...
            <select name="categoryID" required>
                <?php foreach ($categories as $cat): ?>
                    <option value="<?= $cat['categoryID'] ?>" <?= $course['categoryID']==$cat['categoryID']?'selected':'' ?>>
            ...
            <div class="form-row">
                <div>
                    ...
                    <input type="date" name="startDate" id="startDateInput" value="<?= $course['startDate'] ?>" required>
                </div>
                <div>
                    <label>End Date <span class="req">*</span></label>
                    <input type="date" name="endDate" id="endDateInput" value="<?= $course['endDate'] ?>" required>
                </div>
            </div>
            ...
            <div class="form-row">
                <div>
                    ...
                    <input type="time" name="startTime" value="<?= $course['startTime'] ?>" required>
                </div>
                <div>
                    <label>End Time <span class="req">*</span></label>
                    <input type="time" name="endTime" value="<?= $course['endTime'] ?>" required>
                </div>
            </div>
            ...
            <div class="cert-management-box">
                <?php if (!empty($course['certificate_template'])): ?>
                    <span style="color: #27ae60; font-weight: bold;">🟢 Custom Template Uploaded</span>
                    <div class="cert-actions">
                        <a href="../uploads/certificates/<?= $course['certificate_template'] ?>" target="_blank" class="btn-view-pdf">👁️ View Uploaded PDF</a>
                        <a href="edit_training.php?id=<?= $courseID ?>&remove_cert=1" class="btn-remove-cert" onclick="return confirm('Remove custom certificate? System will revert to the default template.')">🗑️ Remove Custom</a>
                    </div>
                <?php else: ?>
                    <span style="color: #7f8c8d; font-weight: bold;">⚪ Using System Default Template</span>
                    <div class="cert-actions">
                        <a href="../certificate/certificate_template.pdf" target="_blank" class="btn-view-pdf">View Default Template</a>
                    </div>
                <?php endif; ?>
            </div>
            ...
            <input type="hidden" name="pastCourses[]" id="pastCoursesHolder" value="<?= implode(',', $selectedPastCourses) ?>">
...
<div class="modal-bg" id="pastModal">
    <div class="modal-box" id="modalBox">
        ...
        <div class="modal-header">
            <h3>Select Past Training Courses</h3>
        ...
        <div id="pastCourseList">
            <?php foreach ($pastCourses as $pc): ?>
                <div class="event-row">
                    <div class="check-col">
                        <input type="checkbox" class="pastCheck" value="<?= $pc['courseID'] ?>"
                        <?= in_array($pc['courseID'], $selectedPastCourses) ? 'checked' : '' ?>>
                    </div>
...
function refreshGalleryDisplay() {
    ...
    for (let i = 0; i < dt.files.length; i++) {
        ...
        reader.onload = function(e) {
            ...
            item.innerHTML = `
                <a href="javascript:void(0)" class="btn-del-img" onclick="removeSelectedFile(${i})">×</a>
...
modalBg.addEventListener("click", function(event) {
    if (!modalBox.contains(event.target)) { closeModal(); }
});
...
if (startInput.value) {
    ...
    deadlineInput.setAttribute('max', startInput.value);
}
</script>
</body>
</html>