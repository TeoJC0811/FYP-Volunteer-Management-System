...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['withdraw_event'])) {
    ...
    if ($update->execute()) {
        ...
        echo "<script>alert('✅ Successfully withdrawn.'); window.location.href='event_detail.php?id=$eventID';</script>";
        exit();
    }
}
...
$stmtGallery->execute();
$galleryImages = $stmtGallery->get_result()->fetch_all(MYSQLI_ASSOC);
...
<style>
/* --- NEW: Admin Preview Banner Style --- */
...
.past-event-card { background: #fff; border: 1px solid #ddd; border-radius: 12px; padding: 15px; display: flex; align-items: flex-start; gap: 20px; text-decoration: none; color: inherit; transition: 0.15s ease;}
</style>
</head>
<body>
...
<div class="main-content">
    ...
    <?php if ($isAdmin && $event['status'] === 'pending'): ?>
        <div class="admin-preview-bar">
            ...
            <div>
                ADMIN PREVIEW: This event is currently PENDING APPROVAL. 
                <br><span style="font-size: 12px; font-weight: normal;">Review details below then return to Admin Panel to Approve or Reject.</span>
            </div>
        </div>
    <?php endif; ?>
                    ...
                    <?php if ($event['status'] === 'approved'): ?>
                        <?php if ($userID): ?>
                            ...
                            <?php elseif ($isRegistered): ?>
                                <form method="POST" onsubmit="return confirm('⚠️ Withdraw? Strikes Left: <?= $strikesLeft ?>.')">
            ...
            <?php while ($review = $reviews->fetch_assoc()): ?>
                <div class="review-card">
                    <div class="review-meta">
                        <span class="review-name"><?= htmlspecialchars($review['userName']) ?></span>
        ...
        <div class="past-event-list">
            ...
            ?>
                <a href="event_detail.php?id=<?= $past['eventID'] ?>" class="past-event-card">
                    ...
                    <div><strong><?= htmlspecialchars($past['eventName']) ?></strong><br><?= date('M Y', strtotime($past['startDate'])) ?></div>
                </a>
            <?php endforeach; ?>
        </div>
    </div>
...
<div id="lightboxOverlay">
    <span id="lightboxClose" onclick="this.parentElement.style.display='none'">×</span>
